...
contract AutoPartNFT_Pro_V2 is ERC721URIStorage, AccessControl, ERC2981 {
    ...
    constructor(address _manufacturer) ERC721("Auto_Part", "APT") {
        if (_manufacturer == address(0)) revert InvalidManufacturer();
        manufacturer = _manufacturer;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MANUFACTURER_ROLE, manufacturer);
        _setDefaultRoyalty(_manufacturer, 200); // 2% fallback
    }


    function createSupplyRequest(bytes32 _productHash, uint256 _quantity) external onlyRole(RETAILER_ROLE) {
        if (_quantity == 0 || _quantity > 100) revert QuantityExceedsOrLess(_quantity);
    ...
    function fulfillSupplyRequest(
        ...
        if (uris.length != req.quantity || metadataHashes.length != req.quantity) revert ArrayLengthMismatch();
    ...
    function _mintToRetailer(address retailer, string memory uri, bytes32 metadataHash)
        internal
        returns (uint256)
    {
    ...
    function batchMintToRetailers(
        ...
        if (retailers.length != uris.length || metadataHashes.length != uris.length) revert ArrayLengthMismatch();
    ...
    function setDefaultRoyalty(address receiver, uint96 feeBasisPoints) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function verifyPartAuthenticity(uint256 tokenId)
        external
        view
        returns (
            bool isAuthentic,
