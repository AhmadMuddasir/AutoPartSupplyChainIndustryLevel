...
//       // Capture the SupplyRequestFulfilled event to get token IDs
...
//       await contract.connect(retailer1).confirmDelivery(tokenId);
//       await contract.connect(retailer1).reportDefectiveReturn(tokenId);
//       // Now manufacturer1 owns the token
...
//       // Token is NEW and owned by retailer1. manufacturer1 does not own it.
//       // Calling repairPart from manufacturer1 should revert with OnlyManufacturer (owner check).
//       await expect(
...
//     beforeEach(async function () {
//       // Setup manufacturer and retailer
//       await contract.connect(manufacturer1).joinAsManufacturer("Tesla", "Austin, TX");
//       // Make manufacturer1 also a retailer so we can mint to them
//       await contract.connect(manufacturer1).requestForRetailer("Tesla Retail", "Austin");
//       await contract.connect(manufacturer1).addRetailer(manufacturer1.address);

//       // Mint a part to manufacturer1 (who is now a retailer)
//       await contract.connect(manufacturer1).mintPartToRetailer(
...
//       // Now manufacturer1 no longer owns it? Actually they still own it? The NFT is still in their wallet, but saleStatus is SOLD.
//       // Transfer should check saleStatus, not ownership.
//       await expect(
...
//       // First, reset the per‑token royalty so the default is used.
//       // We can do that by reporting defective return (which calls _resetTokenRoyalty).
//       // But that would change the token status. We'll just test the default on a token that hasn't been minted? 
//       // Actually we can set default and then check royaltyInfo on a token after reset.
//       // Let's do the flow: sell, return, then check royalty.
//       const phone = "+1234567890";
//       await contract.connect(retailer1).shipPart(tokenId, phone, "TRACK");
//       await contract.connect(retailer1).confirmDelivery(tokenId);
//       await contract.connect(retailer1).reportDefectiveReturn(tokenId);
